/**
* autor: Rodrigo Krummenauer do Nascimento site: www.rkn.com.br email: rodrigoknascimento@gmail.com
* 
* Versão: 3.0 Lincença: GPLv3
*/

/**
* CHANGE LOG
* 
* 2.3 - Problema resolvido que ocorrria no Chrome quando se digitava um mesmo numero 3 vezes na mascara do tipo Money - Adicionada funcionalidade de poder
* iniciar o componente com um valor usando a propriedade value fazendo mascara do valor
* 
* 2.4 - Resolvido BUG do IE, nenhuma mascara funcionava no IE
* 
* 2.5 - Problema ao setvar valores sem decimais resolvido
* 
* 2.6 - Problema de disparar o validade do campo ao apagar tudo selecionando todo o campo e dando um BACKSPACE resolvido.
* 
* 2.7 - Adicionado suporte a propriedade readOnly
* 
* 2.8 - Adicionado Ext.util.Format.MoneyMask
* 
* 2.9 - Adicionada funcionalidade useMask quer ativa ou desativa o uso da classe de mascara.
* 
* 3.0 - Adicionada funcionalidade de negativar ou positivar o valor da mascara monetária.
* 
* 3.1 - Corrigido BUG ao usar mascaras com A (letras maiúsculas ou minúsculas)
*/

/**
* TODO - Selection para o IE - Copy e Paste
* 
*/
...
* 
* Métodos: mask(valor) - Mascara um valor unmask(valor) - Tira a mascara de um valor setMask(mascara) - Define uma nova mascara para o componente
* 
* 
* São 2 parametros (mask, money): mask - Aqui definimos a mascara esta mascara vai ter comportamento diferente dependendo do valor setado no money.
* 
* money - Aqui definimos se teremos uma mascara do tipo dinheiro ou do tipo fixo, true para dinheiro false para normal.
* 
* Mascara Normal: Podemos definir os seguintes caracteres: _ - Qualquer caracter A - Letras MAIUSCULAS ou minusculas L - Letras MAIUSCULAS l - Letras
* minusculas 9 - Números de 0 a 9 <!!> - Expressão regular, exemplo <![0123]!> aceita números de 0 a 3
* 
* Os demais caracteres serão considerados parte da mascara, exemplo: var mask = new Ext.ux.TextMask('999.999.999-99', false); mask.mask('00173008915'); //Deve
* retornar 001.730.089-15 mask.mask('001'); //Deve retornar 001.___.___-__ mask.unmask('001.730.089-15'); //Deve retornar 00173008915
* 
* 
* Mascara Money: Esta mascara funciona basicamente igual a Mascara Normal Podemos definir todos os caracteres da Mascara Normal e mais o 0 e o # os zeros serão
* considerados valor inicial, o # será considerado o fim da mascara considerando da direita pra esquerda permitindo prefixos e o numero de casas decimais será
* contado pela quantidade de caracteres depois do ultimo ponto ou virgula, exemplo: var mask = new Ext.ux.TextMask('R$ #9.999.990,00', true);
* mask.mask(31324587202.18); //Deve retornar R$ 31324.587.202,18 mask.mask(0.01); //Deve retornar R$ 0,01 mask.mask(0); //Deve retornar R$ 0,00 mask.unmask('R$
* 31324.587.202,18'); //Deve retornar 31324587202.18
*/

Ext.ns('Ext.ux.TextMask');

Ext.ux.TextMask = function (mask, money, negative) {
...
Ext.ux.TextMask.prototype = {
    ...
    maskMoney: function (v) {
        ...
        if (this.textMask.indexOf('#') >= 0) {
            m = this.textMask.slice(0, this.textMask.indexOf('#')) + (negativo && this.negative ? '-' : '') + m;
        }
        return m;
    },
    unmask: function (v) {
...
* 
* var campo = new Ext.ux.form.MaskTextField({ mask: '(099) 9999-9999', money: false })
* 
* Temos a função setMask(mascara) que serve para mudar a mascara depois que o objeto já estiver criado.
*/
Ext.ns('Ext.ux.form');
Ext.ux.form.MaskTextField = Ext.extend(Ext.form.TextField, {
    ...
    initComponent: function () {
        ...
        this.textMask = new Ext.ux.TextMask(this.mask, this.money, this.allowNegative);
    },
    onRender: function () {
        ...
        this.el.on({
            ...
            paste: function (e) {
                ...
                this.el.dom.value = '';
                this.validatePaste.defer(10, this, [e]);
            },
            scope: this
        });
    ...
    validatePaste: function (e) {
        this.hiddenField.dom.value = this.textMask.unmask(this.el.dom.value);
        this.el.dom.value = this.textMask.mask(this.hiddenField.dom.value);
    },
    getKeyCode: function (onKeyDownEvent, type) {
        ...
        keycode.isDelete = ((onKeyDownEvent.getKey() == Ext.EventObject.DELETE && type == 'keydown') || (type == 'keypress' && onKeyDownEvent.charCode === 0 && onKeyDownEvent.keyCode == Ext.EventObject.DELETE))
				? true
				: false;
    ...
    updateHidden: function (e) {
        ...
        if (!(kk >= e.F1 && kk <= e.F12) && !e.isNavKeyPress()) {
            if (this.el.dom.value.length == 0 || (this.el.dom.selectionStart == 0 && this.el.dom.selectionEnd == this.el.dom.value.length)) {
    ...
    getValue: function () {
        var val = this.hiddenField && this.hiddenField.dom ? this.hiddenField.dom.value : this.textMask.unmask(this.value);
        return this.money ? parseFloat(val) : val;
    },
    ...
    getErrors: function (value) {
        var errors = Ext.ux.form.MaskTextField.superclass.getErrors.apply(this, arguments);

        if (this.money) {
            if (!this.allowBlank && (value == 0)) {
...
* 
* var campo = new Ext.ux.form.MaskTextField({ //Não precisamos definir nada, o componente mascara de acordo com o formato //da data que já está definido no
* componente. })
*/
...
/**
* ATENÇÃO Usado pra substituir todos os datefields que uso pelo datefield com mascara isto ocorre sobreescrevendo o xtype do datefield normal, faço isso pq uso
* somente xtypes no meu sistema, caso não queira isto apague a linha abaixo.
*/
// TODO - jpimentel
// Ext.reg('datefield', Ext.ux.form.MaskDateField);
/**
* Aqui temos um Sigleton para a mascara, use para formatações rapidas, exemplo Ext.util.Format.TextMask.setMask('99/99/9999').mask('10102010'); //Vai retornar
* 10/10/2010
*/
Ext.util.Format.TextMask = new Ext.ux.TextMask();

/**
* Aqui temos um Sigleton para a mascara de valores, use para formatações rapidas, exemplo Ext.util.Format.MoneyMask.setMask('R$#9.990,00').mask('100'); //Vai
* retornar R$100,00
*/
Ext.util.Format.MoneyMask = new Ext.ux.TextMask('', true);

/**
* Aqui temos um renderer pra colunas de um grid { header: 'Telefone', dataIndex: 'fone', renderer: Ext.util.Format.maskRenderer('(099) 9999-9999') }
*/
